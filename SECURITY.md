...
- **Do not expose port 8000 to the internet without additional firewall rules.** By default the server binds to `127.0.0.1` (loopback only). Set `PR_HOST=0.0.0.0` only if you intend remote access and have network-level guards in place.
- **Set `PR_API_KEY`** before sharing access with anyone. See [README.md](README.md) for setup instructions.
...
- [ ] `PR_API_KEY` is set to a strong random value (`python3 -c "import secrets; print(secrets.token_hex(32))"`)
- [ ] `PR_CORS_ORIGINS` is set to your specific frontend origin (not left empty)
